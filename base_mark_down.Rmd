...
Is there a relationship between the day of the week and the steps that you do?
If so, we are going to try to make a predictor for it.

```{r}
step_data = read.csv("Step_Data.csv")
steps = step_data$Steps
weekday = step_data$dayweek
weekday = step_data$Weekday+1
```

We define the model in `rjags` by stating the likelihood model for $Y$ and prior distributions for all required parameters. Given that our data is discrete, we apply a poisson distribution 
...
regress_model <- "model{
    ...
    a ~ dunif(100, 12000)
    b[1] <- 0  # sunday
    b[2] ~ dnorm(0, 600^(-2))#weekend # monday-sunday
    b[3] ~ dnorm(0, 600^(-2)) # tuesday - sunday
    b[4] ~ dnorm(0, 600^(-2)) # wensday - sunday
    b[5] ~ dnorm(0, 600^(-2)) # thrusday - sunday
    b[6] ~ dnorm(0, 600^(-2)) # friday - sunday
    b[7] ~ dnorm(0, 600^(-2)) # saturday - sunday
...
We compile model adjusting to our data. We are predicting steps per day, so we use the steps as Y and weekday as X and burn in the model
...
regress_jags <- jags.model(textConnection(regress_model), 
                        data = list(Y = steps, X = weekday), n.chains = 4)
regress_sim <- update(regress_jags, n.iter = 10000)

```


Simulate from the model, summarising the posteriors and checking convergence

```{r, question2d}
# SIMULATE the posterior    
regress_sim <- coda.samples(model=regress_jags,
...
df = data.frame(weekend = regress_sim[[1]][,1], 
                b = regress_sim[[1]][,3])
df$weekday = df$var1 + df$var1.1
df$weekend = df$var1

ggplot(df, aes(weekend)) + geom_boxplot() + geom_boxplot(aes(weekday))
```
```{r}
gelman.diag(regress_sim[,c(1,3,4,5,6,7,8)])
gelman.plot(regress_sim[,c(1,3,4,5,6,7,8)])
```